...
def find_image_path(directory, base_name):
    """Search for files matching the given base name in the directory."""
    for file in os.listdir(directory):
        if file.startswith(base_name) and file.split('.')[-1] in ['png', 'jpg', 'jpeg']:
            return os.path.join(directory, file)
    return None

def main():
    # Hardcoded file paths for base64 text files
    ...
    # Decode the base64 strings to images
    decode_base64_to_image(input_text_path_1, output_image_path_1)
    decode_base64_to_image(input_text_path_2, output_image_path_2)

    # After decoding, perform face comparison
    directory = os.getcwd()  # Get the current working directory
    img1_path = find_image_path(directory, 'img1')
    img2_path = find_image_path(directory, 'img2')

    if img1_path and img2_path:
        print("Found Image 1 at:", img1_path)
        print("Found Image 2 at:", img2_path)

        # Verify if images belong to the same person
        result = DeepFace.verify(img1_path, img2_path)
        print("Is same face:", result['verified'])
    else:
        print("One or both images not found in the specified directory.")
